...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.delete("/api/classes/:id", async (req, res) => {
    ...
    try {
      ...
      const assignments = await storage.getAssignmentsByClassId(classId);
      
      if (assignments.length > 0) {
        return res.status(400).json({ 
          error: "Cannot delete class with teacher assignments. Please remove assignments first." 
        });
      }
  ...
  app.get("/api/teacher-assignments", async (req, res) => {
    try {
      const assignments = await storage.getAllTeacherClassAssignments();
  ...
  app.get("/api/teacher-assignments/teacher/:teacherId", async (req, res) => {
    try {
      const teacherId = Number(req.params.teacherId);
      const assignments = await storage.getAssignmentsByTeacherId(teacherId);
      res.json(assignments);
    } catch (error) {
      console.error("Error fetching teacher assignments:", error);
      res.status(500).json({ error: "Failed to fetch teacher assignments" });
  ...
  app.get("/api/teacher-assignments/class/:classId", async (req, res) => {
    try {
  ...
  app.post("/api/teacher-assignments", async (req, res) => {
    if (!req.isAuthenticated() || req.user.role !== "admin") {
      return res.status(403).json({ error: "Only admins can create teacher assignments" });
    }
    
    try {
      // Check if teacher exists and is a teacher
      const teacher = await storage.getUser(req.body.teacherId);
      if (!teacher || teacher.role !== "teacher") {
        return res.status(400).json({ error: "Invalid teacher ID" });
      }
      
      // Check if class exists
      const classData = await storage.getClass(req.body.classId);
      if (!classData) {
        return res.status(400).json({ error: "Invalid class ID" });
      }
      
      // Check for existing assignment
      const existingAssignments = await storage.getAssignmentsByClassId(req.body.classId);
      const existingForTeacher = existingAssignments.find(a => a.teacherId === req.body.teacherId);
      ...
      // If setting as main teacher, check if there's already a main teacher
      if (req.body.isMainTeacher) {
        const existingMainTeacher = existingAssignments.find(a => a.isMainTeacher);
        if (existingMainTeacher) {
          return res.status(400).json({ 
            error: "This class already has a main teacher. Please update the existing assignment first." 
          });
        }
      }
  ...
  app.patch("/api/teacher-assignments/:id", async (req, res) => {
    if (!req.isAuthenticated() || req.user.role !== "admin") {
      return res.status(403).json({ error: "Only admins can update teacher assignments" });
    }
    
    try {
      const assignmentId = Number(req.params.id);
      const assignment = await storage.getTeacherClassAssignment(assignmentId);
      
      if (!assignment) {
        return res.status(404).json({ error: "Teacher assignment not found" });
      }
      
      // If updating to main teacher, check if there's already another main teacher
      if (req.body.isMainTeacher) {
        const existingAssignments = await storage.getAssignmentsByClassId(assignment.classId);
        const existingMainTeacher = existingAssignments.find(
          a => a.isMainTeacher && a.id !== assignmentId
        );
      ...
      const updatedAssignment = await storage.updateTeacherClassAssignment(assignmentId, req.body);
  ...
  app.delete("/api/teacher-assignments/:id", async (req, res) => {
    if (!req.isAuthenticated() || req.user.role !== "admin") {
      return res.status(403).json({ error: "Only admins can delete teacher assignments" });
    }
    ...
    try {
      ...
      const deleted = await storage.deleteTeacherClassAssignment(assignmentId);
  ...
  // Endpoint to get all students in a class (based on all teachers assigned to the class)
  app.get("/api/classes/:id/students", async (req, res) => {
    try {
      ...
      // Get all students with classId matching this class
      const students = await storage.getAllUsers();
      const classStudents = students.filter(u => 
        u.role === "student" && u.classId === classId
      );
  ...
  app.post("/api/classes/:id/add-student/:studentId", async (req, res) => {
    if (!req.isAuthenticated() || req.user.role !== "admin") {
      return res.status(403).json({ error: "Only admins can add students to classes" });
    }
    ...
    try {
      ...
      const updatedStudent = await storage.updateUser(studentId, { classId });
  ...
  app.delete("/api/classes/:id/remove-student/:studentId", async (req, res) => {
    if (!req.isAuthenticated() || req.user.role !== "admin") {
      return res.status(403).json({ error: "Only admins can remove students from classes" });
    }
    ...
    try {
      ...
      // Check if student exists, is a student, and is in this class
      const student = await storage.getUser(studentId);
      if (!student || student.role !== "student") {
        return res.status(404).json({ error: "Student not found" });
      }
      ...
      // Remove the student from the class by setting classId to null
      const updatedStudent = await storage.updateUser(studentId, { classId: null });
  ...
  app.post("/api/classes/:id/add-students", async (req, res) => {
    if (!req.isAuthenticated() || req.user.role !== "admin") {
      return res.status(403).json({ error: "Only admins can add students to classes" });
    }
    ...
    try {
      ...
      res.status(200).json({
        success: true,
        message: `${updated.filter(Boolean).length} students added to class successfully`,
        addedStudents: updated.filter(Boolean)
      });
    } catch (error) {
